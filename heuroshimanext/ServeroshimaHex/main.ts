...
import { responseFrom, Game, positive, negative, Player, GameHex } from "./classes"
import {faker} from "@faker-js/faker"
import { EntityType, TileEntity, direction, ActiveCard, InstantAction, EntityActionType } from "../unitTypes"
import { Ok, Err, Result, unwrap } from "../RustlikeTypes"
...
class NetworkGameFacade {
    ...
    war() {
        ...
        const getTile = (x: number, y: number): Result<GameHex> =>  {
            let res = this.game.board.hexes.find(hex => 
                hex.coords.x == x &&
                hex.coords.y == y
            )
            if (res) {
            ...
            return Err(`Tile ${x}, ${y} doesn't exist`)
        }
            ...
            for (let action of entity.actions) {
                ...
                switch(action.type) {
                    ...
                    case EntityActionType.meele: {
                        const res = getTile(entity.position.x + direction.x, entity.position.y + direction.y)
                        const target = unwrap(res)
                        if(!target || !target.tileEntity) {
                            break;
                        }
                        if (target.owner?.id != entity.owner.id && target.tileEntity?.type == EntityType.Base)
                            entity.owner.score += 1
                        let hitDirection = (entity.rotation + action.direction + 3) % HexSideCount
                        let responses = target.tileEntity.actions.filter(action => (action.direction + target.rotation) % HexSideCount == hitDirection)
                        if (!responses.find(action => action.type == EntityActionType.block)) {
                    ...
                    case EntityActionType.ranged: {
                        //5 as max map size
                        for (let n = 1; n < 5; n++) {
                            const res = getTile(entity.position.x + direction.x * n, entity.position.y + direction.y * n)
                            const target = unwrap(res)
                            if(!target)
                                break;
                            const targetOwnerId = target.owner?.id
                            if (!targetOwnerId || !target.tileEntity) {
                                continue;
                            }
                            if (targetOwnerId != entity.owner.id && target.tileEntity.type == EntityType.Base)
                                entity.owner.score += 1
                            let hitDirection = (entity.rotation + action.direction + 3) % HexSideCount
                            let responses = target.tileEntity.actions.filter(
                                action => (action.direction + target.rotation) % HexSideCount == hitDirection
                            )
                            applyDamage(target.coords, 1)
                            if (responses.find(action => action.type == EntityActionType.block)) {
                                break;
                    ...
                    for (let n = 1; n < 5; n++) {
                        ...
                        if (target.tileEntity?.type == EntityType.Base)
                            entity.owner.score += 1
                        if(target.owner?.id != entity.owner.id)
                            applyDamage(target.coords, 1)
                        }
                    }
                }
    ...
    build(socket: Socket, tile: TileInterface): Result<TileInterface> {
        ...
        //
        if (!player.basePlaced && tile.tileEntity?.type != EntityType.Base) {
...
gameNamespace.on("connection", async (socket: MySocket) => {
    ...
    socket.on("req:use_action", (action: InstantAction, coords: coords) => {
        ...
        gameNamespace.to(socket.game!.id).emit("broad:sync_players", players)
        socket.emit("broad:sync_players", players)
